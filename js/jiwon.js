...
function updateSlider(value) {
  ...
  const transition = svg.transition().duration(1000).ease(d3.easeCubic);
  ...
  if (isCumulative) {
    ...
    data = restData.filter((d) => {
      ...
      return (
        rowDate < sliderDate || rowDate.getMonth() === sliderDate.getMonth()
      );
    });
  } else {
    // non-cumulative
    data = restData.filter(
      (d) => d["Date of Challenge/Removal"] === sliderMonthYear
    );
  ...
  const hi = d3.select("#x-grid").transition().duration(1000).call(xGrid);
  console.log(hi);
  ...
  d3.select("#monthYearLabel").text(
    formatMonthYearForBackgroundLabel(sliderDate)
  );
}
...
function initializeElements() {
  ...
  data = restData.filter(
    (d) =>
      d["Date of Challenge/Removal"] === sliderValueToMonthYear[curMonthYear]
  );
  var numBooksDataEntries = updateAndGetData()
    .sort((a, b) => b.count - a.count)
    .slice(0, 10)
    .reverse();
...
function updateToolTip(state, numBans) {
  ...
  if (!isCumulative || curMonthYear === 0) {
    ...
    numBansTextDate.innerHTML =
      "by " + sliderValueToMonthYear[curMonthYear] + " since July 2021";
  }
}
